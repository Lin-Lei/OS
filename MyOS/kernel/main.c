...
/* Remove User */
void removeUser(char * username, char * password)
{
	if(UserState == 3){
		int i = 0;
		for (i = 0; i < 2; i++)
...
void playchess(int fd_stdin,int fd_stdout)
{
    ...
    do{
        ...
        if(buf[0]=='q')
            return;
        while(k!='y'&&k!='n'){
            printf("Input error, please check and enter again.\n");
            read(fd_stdin,buf,2);
            if(buf[0]=='q')
                return;
            k = buf[0];
...
int start(int fd_stdin,int fd_stdout)
{
    ...
    while(x!=0){
        ...
        while((a<0||a>9)||(b<0||b>9)||chess[b][a]!=SPA) {
            printf("Input error, please check and answer again.");
            int i=0,j=0;
            char xa[]={0,0,0};
            char yb[]={0,0,0};
            int r = read(fd_stdin, buf, 10);
